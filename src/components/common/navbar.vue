        ...
        <md-theme :md-name="theme">
            ...
            <transition name="slideD">
                <md-bottom-bar md-shift class="btm-nav">
                    <md-bottom-bar-item @click.native="doAction(0)" :class="{'md-active': isActive[0]}"><i
                            class="iconfont icon-tubiao- btm-nav-icon"></i>电影
                    </md-bottom-bar-item>
                    <md-bottom-bar-item @click.native="doAction(1)" :class="{'md-active': isActive[1]}"><i
                            class="iconfont icon-music btm-nav-icon"></i>音乐
                    </md-bottom-bar-item>
                    <md-bottom-bar-item @click.native="doAction(2)" :class="{'md-active': isActive[2]}"><i
                            class="iconfont icon-book btm-nav-icon"></i>书籍
                    </md-bottom-bar-item>
                    <md-bottom-bar-item @click.native="doAction(3)" :class="{'md-active': isActive[3]}"><i
                            class="iconfont icon-photo btm-nav-icon"></i>图片
                    </md-bottom-bar-item>
                </md-bottom-bar>
            </transition>
        </md-theme>
...
<script>
    ...
    export default {
        mounted: function () {
            let scrT = document.body.scrollTop || window.scrollY;
            window.addEventListener("scroll", Util.debounce(function () {
                let newScrT = document.body.scrollTop || window.scrollY;
