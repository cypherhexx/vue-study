<template>
    <div id="app">
        ...
        <md-dialog-alert :md-content="alert.content" :md-ok-text="alert.ok" @open="onOpen" @close="onClose" ref="info">
        </md-dialog-alert>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            formatTime(val) {
                let m = Math.floor(val / 60).toString();
                let s = Math.round(val % 60).toString();
                m = (m.length == 1) ? 0 + m : m;
                s = (s.length == 1) ? 0 + s : s;
                return m + ":" + s;
            },
